...
//!   A. untimed   -> trades.csv + digest.txt   (correctness / golden artifacts)
//!   B. timed     -> latencies.csv             (per-op latency distribution)
...
fn main() -> std::io::Result<()> {
    ...
    book.reserve_trades(n);
    for i in 0..n {
        book.process(&decode(i));
    }
    let digest = book.digest();
    let mut tw = BufWriter::new(File::create(format!("{out_dir}/trades.csv"))?);
    writeln!(tw, "seq,maker_id,taker_id,price,qty,aggressor")?;
    const PRIME: u64 = 0x100000001b3;
    let mut thash: u64 = 0xcbf29ce484222325;
    for (seq, t) in book.trades.iter().enumerate() {
        for v in [t.maker, t.taker, t.price, t.qty] {
            for b in v.to_le_bytes() {
                thash ^= b as u64;
                thash = thash.wrapping_mul(PRIME);
            }
        }
        thash ^= t.aggressor as u64;
        thash = thash.wrapping_mul(PRIME);
        let ag = if t.aggressor == lob::SIDE_BID { 'B' } else { 'S' };
        writeln!(tw, "{seq},{},{},{},{},{ag}", t.maker, t.taker, t.price, t.qty)?;
    }
    tw.flush()?;
    let trade_count = book.trades.len();
    fs::write(
        format!("{out_dir}/digest.txt"),
        format!("trades={trade_count}\ntrades_hash={thash:016x}\nbook_digest={digest:016x}\n"),
    )?;
    ...
    eprintln!("trades_hash      {thash:016x}");
    ...
    eprintln!("latency ns  p50={} p99={} p999={} max={}", pct(0.50), pct(0.99), pct(0.999), s[s.len() - 1]);
    Ok(())
}
